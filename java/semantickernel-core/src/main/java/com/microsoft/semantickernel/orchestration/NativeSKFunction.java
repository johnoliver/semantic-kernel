...
public class NativeSKFunction extends AbstractSkFunction<Void, SemanticSKContext> {
  ...
  private static MethodDetails getMethodDetails(
    ...
    return new MethodDetails(hasSkFunctionAttribute, type, function, parameters, name, description);
  }
  ...
  private static String getArgumentValue(
      Method method, SemanticSKContext context, Parameter parameter, Set<Parameter> inputArgs) {
            ...
            .filter(
                it ->
                    !(it.isAnnotationPresent(SKFunctionParameters.class)
                        && !it.getAnnotation(SKFunctionParameters.class).name().isEmpty()))
            .collect(Collectors.toList());
