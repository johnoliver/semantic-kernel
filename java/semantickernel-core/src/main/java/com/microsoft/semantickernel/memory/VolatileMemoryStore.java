...
public class VolatileMemoryStore implements MemoryStore {
	...
	private static final ToDoubleFunction<Tuple2<MemoryRecord, ? extends Number>> extractSimilarity = tuple -> tuple
		.getT2().doubleValue();
	...
	public Mono<Collection<Tuple2<MemoryRecord, Number>>> getNearestMatchesAsync(
		...
		// Upcast the embedding vector to a Number to get around compiler complaining
		// about type
		// mismatches.
		...
		embeddingCollection.forEach(
			record -> {
				if (record != null) {
					EmbeddingVector<Number> recordVector = upcastEmbeddingVector(record.getEmbedding().getVector());
