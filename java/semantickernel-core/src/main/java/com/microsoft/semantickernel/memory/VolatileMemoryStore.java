...
public class VolatileMemoryStore implements MemoryStore {
      ...
      if (withEmbedding) {
        ...
        return Mono.just(
            MemoryRecord.fromMetadata(
                record.getMetadata(), null, record.getMetadata().getId(), record.getTimestamp()));
      }
    }
    return Mono.empty();
  }
  ...
  private static final ToDoubleFunction<Tuple2<MemoryRecord, ? extends Number>> extractSimilarity =
      tuple -> tuple.getT2().doubleValue();
  ...
  protected Map<String, MemoryRecord> getCollection(@Nonnull String collectionName) {
    ...
    if (collection == null) {
      throw new MemoryException(
          MemoryException.ErrorCodes.ATTEMPTED_TO_ACCESS_NONEXISTENT_COLLECTION, collectionName);
    }
    return collection;
  }
