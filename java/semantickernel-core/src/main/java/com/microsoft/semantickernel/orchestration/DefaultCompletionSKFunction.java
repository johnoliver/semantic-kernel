...
public class DefaultCompletionSKFunction
  ...
  public DefaultCompletionSKFunction(
    ...
    super(delegateTypes, parameters, skillName, functionName, description, kernelSkillsSupplier);
    // TODO
  ...
  */
  ...
  public Mono<CompletionSKContext> aggregatePartitionedResultsAsync(
    ...
    return results
        .map(list -> list.stream().map(SKContext::getResult).collect(Collectors.joining("\n")))
        .map(context::update);
  }
  ...
  protected Mono<CompletionSKContext> invokeAsyncInternal(
    ...
    return function
        .run(this.aiService.get(), finalSettings, context)
        .map(
            result -> {
  ...
  /// <returns>SK function instance.</returns>
  public static DefaultCompletionSKFunction createFunction(
      @Nullable String skillNameFinal, String functionName, SemanticFunctionConfig functionConfig) {
