  ...
  public DefaultSkillCollection(Map<String, ? extends ReadOnlyFunctionCollection> skillCollection) {
    ...
    Map<String, FunctionCollection> cloned =
        skillCollection.entrySet().stream()
            .collect(
                Collectors
                    .<Map.Entry<String, ? extends ReadOnlyFunctionCollection>, String,
                        FunctionCollection>
                        toMap(Map.Entry::getKey, it -> new FunctionCollection(it.getValue())));
    ...
    in.asMap()
        ...
        .forEach(
            entry -> {
              ...
              if (existing == null) {
                skillCollection.put(
                    entry.getKey().toLowerCase(), new FunctionCollection(entry.getValue()));
              } else {
