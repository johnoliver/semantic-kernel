...
public class DefaultSkillCollection implements ReadOnlySkillCollection {
	...
	public DefaultSkillCollection(
		...
		Map<String, FunctionCollection> cloned = skillCollection.entrySet().stream()
			.collect(
				Collectors.<Map.Entry<String, ? extends ReadOnlyFunctionCollection>, String, FunctionCollection>toMap(
					Map.Entry::getKey,
					it -> new FunctionCollection(
						it.getValue())));
	...
	public DefaultSkillCollection merge(DefaultSkillCollection in) {
		in.asMap()
			...
			.forEach(
				entry -> {
					FunctionCollection existing = skillCollection.get(entry.getKey().toLowerCase());
					if (existing == null) {
						skillCollection.put(
