...
public class DefaultSemanticTextMemory implements SemanticTextMemory {
	...
	@Nonnull
	private /* final */ MemoryStore _storage;
	...
	public DefaultSemanticTextMemory(
		...
		// TODO: this assignment raises EI_EXPOSE_REP2 in spotbugs (filtered out for
		// now)
		...
		return _embeddingGenerator
			...
			.flatMap(
				embeddings -> {
					...
					MemoryRecordMetadata data = new MemoryRecordMetadata(
	...
	private static final Function<Collection<Tuple2<MemoryRecord, Number>>, Mono<List<MemoryQueryResult>>> transformMatchesToResults = records -> {
		if (records.isEmpty()) {
			return Mono.empty();
		}
		return Mono.just(
			records.stream()
				.map(
					record -> {
						Tuple2<MemoryRecord, ? extends Number> tuple = record;
	...
	public Mono<String> saveReferenceAsync(
		...
		// }
		//
		// return await this._storage.UpsertAsync(collection, record: data, cancel:
		// cancel);
	}
