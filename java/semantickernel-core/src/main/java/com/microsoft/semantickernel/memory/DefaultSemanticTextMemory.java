...
public class DefaultSemanticTextMemory implements SemanticTextMemory {
        ...
        .flatMap(
            embeddings -> {
              ...
              MemoryRecordMetadata data =
                  new MemoryRecordMetadata(true, id, text, description, "", additionalMetadata);
              MemoryRecord memoryRecord =
                  new MemoryRecord(data, embeddings.iterator().next(), collection, null);
  ...
  public Mono<MemoryQueryResult> getAsync(String collection, String key, boolean withEmbedding) {
    return _storage
        .getAsync(collection, key, withEmbedding)
                    ...
                    .map(
                        record -> {
                          ...
                          return new MemoryQueryResult(
                              memoryRecord.getMetadata(), relevanceScore.doubleValue());
                        })
