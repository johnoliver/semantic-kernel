...
public class DefaultKernel implements Kernel {
	...
	public <T> T getService(String serviceId, Class<T> clazz) {
		if (TextCompletion.class.isAssignableFrom(clazz)) {
			Function<Kernel, TextCompletion> factory = kernelConfig.getTextCompletionServiceOrDefault(serviceId);
			if (factory == null) {
			...
			Function<Kernel, ChatCompletion> factory = kernelConfig.getChatCompletionServiceOrDefault(serviceId);
			if (factory == null) {
				throw new KernelException(
					KernelException.ErrorCodes.ServiceNotFound,
	...
	@Override
	public <RequestConfiguration, FunctionType extends SKFunction<RequestConfiguration>> FunctionType registerSemanticFunction(
		FunctionType func) {
	...
	/// <summary>
	/// Import a set of functions from the given skill. The functions must have the
	/// `SKFunction`
	// attribute.
	/// Once these functions are imported, the prompt templates can use functions to
	/// import content
	// at runtime.
	/// </summary>
	/// <param name="skillInstance">Instance of a class containing functions</param>
	/// <param name="skillName">Name of the skill for skill collection and prompt
	/// templates. If the
	// value is empty functions are registered in the global namespace.</param>
	/// <returns>A list of all the semantic functions found in the directory,
	/// indexed by function
	// name.</returns>
	@Override
	...
	public ReadOnlyFunctionCollection importSkill(
		...
		// skill = new Dictionary<string,
		// ISKFunction>(StringComparer.OrdinalIgnoreCase);
		ReadOnlyFunctionCollection functions = SkillImporter.importSkill(skillInstance, skillName,
			() -> defaultSkillCollection);

		DefaultSkillCollection newSkills = functions.getAll().stream()
			.reduce(
	...
	public ReadOnlyFunctionCollection importSkillFromDirectory(
		...
		Map<String, SemanticFunctionConfig> skills = KernelExtensions.importSemanticSkillFromDirectory(
	...
	public Mono<SKContext> runAsync(ContextVariables variables, SKFunction<?>... pipeline) {
		...
		// TODO: The SemanticTextMemory can be null, but there should be a way to
		// provide it.
		// Not sure registerMemory is the right way.
		Mono<SKContext> pipelineBuilder = Mono.just(pipeline[0].buildContext(variables, null, getSkills()));

		for (SKFunction f : Arrays.asList(pipeline)) {
			pipelineBuilder = pipelineBuilder.flatMap(
				newContext -> {
					SKContext context = f.buildContext(
