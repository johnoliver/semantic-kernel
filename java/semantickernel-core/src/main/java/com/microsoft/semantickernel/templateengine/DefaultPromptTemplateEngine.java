        ...
        .flatMap(
            block -> {
              if (block instanceof TextRendering) {
                return Mono.just(((TextRendering) block).render(context.getVariables()));
              } else if (block instanceof CodeRendering) {
                return ((CodeBlock) block).renderCodeAsync(context);
              } else {
                String message =
                    "Unexpected block type, the block doesn't have a rendering" + " method";
                LOGGER.error(message);
                return Mono.error(
                    new TemplateException(
                        TemplateException.ErrorCodes.UnexpectedBlockType, message));
              }
            })
        .collectList()
        .map(
