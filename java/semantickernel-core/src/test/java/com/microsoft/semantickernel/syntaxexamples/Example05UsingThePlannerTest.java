...
public class Example05UsingThePlannerTest {
  ...
  public static SequentialPlanner getPlanner(Kernel kernel) {
    ...
    kernel.importSkill(
        "WriterSkill",
        KernelExtensions.importSemanticSkillFromDirectory("../../samples/skills", "WriterSkill"));
  ...
  public void run() {
    ArgumentMatcher<String> matcher =
        prompt -> {
          return prompt.contains("Create an XML plan step by step, to satisfy the goal given");
        };
    ...
    System.out.println(
        planner
            .createPlanAsync("Write a poem about John Doe, then translate it into Italian.")
            .block()
            .getResult());
  }
}
