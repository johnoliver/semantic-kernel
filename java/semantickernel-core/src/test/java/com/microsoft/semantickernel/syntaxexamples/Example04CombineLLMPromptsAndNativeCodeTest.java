  ...
  public void run() {
    OpenAIAsyncClient client =
        mockCompletionOpenAIAsyncClient(
            Tuples.of("Gran Torre Santiago is the tallest building in South America", "A-SUMMARY"));

    KernelConfig kernelConfig =
        SKBuilders.kernelConfig()
            .addTextCompletionService(
                "text-davinci-002", kernel -> new OpenAITextCompletion(client, "text-davinci-002"))
            .addTextCompletionService(
                "text-davinci-003", kernel -> new OpenAITextCompletion(client, "text-davinci-003"))
    ...
    Assertions.assertEquals(
        "Gran Torre Santiago is the tallest building in South America", result.block().getResult());
