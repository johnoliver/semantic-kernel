...
public class DefaultKernelTest {
  ...
  void contextVariableTest() {
    ...
    OpenAIAsyncClient client =
        mockCompletionOpenAIAsyncClient(Tuples.of("A", "a"), Tuples.of("user: Aauser: B", "b"));
    ...
    CompletionSKFunction chat =
        kernel
            ...
            .createFunction(
                ...
                null,
                null,
                new PromptTemplateConfig.CompletionConfig(0.7, 0.5, 0, 0, 2000, new ArrayList<>()));
    ...
    result =
        result
            .appendToVariable("history", "user: A" + result.getResult())
  ...
  public static Kernel buildKernel(String model, OpenAIAsyncClient client) {
    ...
    KernelConfig kernelConfig =
        SKBuilders.kernelConfig().addTextCompletionService(model, kernel -> textCompletion).build();
            ...
            .map(
                response ->
                    Tuples.<ArgumentMatcher<String>, String>of(
                        arg -> arg.contains(response.getT1()), response.getT2()))
            ...
            Mockito.<CompletionsOptions>argThat(
                completionsOptions ->
                    completionsOptions.getPrompt().size() == 1
                        && completionsOptions.getPrompt().get(0).equals(expected)));
  }
}
