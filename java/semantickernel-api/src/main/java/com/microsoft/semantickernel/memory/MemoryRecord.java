...
/**
 * A record in a Semantic Kernel memory. IMPORTANT: this is a storage schema.
 * Changing the fields
...
public class MemoryRecord extends DataEntryBase {
	...
	/**
	 * Prepare an instance about a memory which source is stored externally. The
	 * universal resource
	 ...
	 * @param sourceName
	 *         Name of the external service, e.g. "MSTeams", "GitHub", "WebSite",
	 *         "Outlook
	 *         IMAP", etc.
	 * @param description
	 *         Optional description of the record. Note: the description is not
	 *         indexed.
	 * @param embedding
	 ...
	 *
	 * @param id
	 *         Resource identifier within the storage provider, e.g. record
	 *         ID/GUID/incremental
	 *         counter etc.
	 * @param text
	...
	public boolean equals(Object o) {
		...
		if (!embedding.equals(that.embedding))
			return false;
	 ...
	 * /// </summary>
	 * /// <param name="json">Json string representing a memory record's
	 * metadata.</param>
	 * /// <param name="embedding">Optional embedding associated with a memory
	 * record.</param>
	 ...
	 * if (metadata != null)
	 * {
	 * return new MemoryRecord(metadata, embedding ?? Embedding<float>.Empty, key,
	 * timestamp);
	 * }
	 * 
	 ...
	 * /// </summary>
	 * /// <returns>The memory record's metadata serialized to a json
	 * string.</returns>
