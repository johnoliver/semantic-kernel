            ...
            try {
                ...
                return r
                    .map(it -> {
                        ...
                        // If the function has a return type that has a ContextVariableType<T>, use
                        // it.
                        ContextVariableType<T> contextVariableType = getContextVariableType(
                            context,
                            returnParameterType);
                        if (contextVariableType == null) {
                            // If getting the context variable type from the function fails, default
                            // to
    ...
    private static Object getArgumentValue(
        ...
        if (arg == null) {
            ...
            if (annotation != null) {
                ...
                // Expectation here is that the fromPromptString method will be able to handle a
