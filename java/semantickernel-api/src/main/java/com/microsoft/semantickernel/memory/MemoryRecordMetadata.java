...
public class MemoryRecordMetadata {
	 ...
	 * storage provider
	 * or is available through and external service, such as web site, MS Graph,
	 * etc.
	 */
	...
	/**
	 * A value used to understand which external service owns the data, to avoid
	 * storing the
	 * information inside the URI. E.g. this could be "MSTeams", "WebSite",
	 * "GitHub", etc.
	 *
	...
	/**
	 * Unique identifier. The format of the value is domain specific, so it can be a
	 * URL, a GUID,
	 * etc.
	 *
	 ...
	 *
	 * @param isReference
	 *         True if source data is local, false if source data comes from an
	 *         external
	 *         service.
	 * @param id
	 *         Unique string used to identify the {@code MemoryRecord}.
	 * @param text
	 *         Local source data associated with a {@code MemoryRecord}
	 *         embedding.
	 * @param description
	 *         {@code MemoryRecord} description.
	...
	public boolean equals(Object o) {
		...
		if (isReference != that.isReference)
			return false;
		if (!externalSourceName.equals(that.externalSourceName))
			return false;
