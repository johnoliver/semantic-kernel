...
public interface MemoryStore {
  ...
  /**
   * Upserts a memory record into the data store. Does not guarantee that the collection exists. If
   * the record already exists, it will be updated. If the record does not exist, it will be
   * created.
   *
   * @param collectionName The name associated with a collection of embeddings.
   ...
   * exists. If the record already exists, it will be updated. If the record does not exist, it will
   * be created.
   *
  ...
  Mono<Collection<MemoryRecord>> getBatchAsync(
      @Nonnull String collectionName, @Nonnull Collection<String> keys, boolean withEmbeddings);
  ...
  /**
   * Gets the nearest match to the {@link Embedding} of type {@code Float}. Does not guarantee that
