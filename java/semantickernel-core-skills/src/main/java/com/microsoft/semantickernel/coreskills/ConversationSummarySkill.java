...
public class ConversationSummarySkill {
	...
	public ConversationSummarySkill(Kernel kernel) {
		this.kernel = kernel;
		this.summarizeConversationFunction = SKBuilders.completionFunctions(kernel)
			.createFunction(
		...
		this.conversationActionItemsFunction = SKBuilders.completionFunctions(kernel)
			.createFunction(
		...
		this.conversationTopicsFunction = SKBuilders.completionFunctions(kernel)
			.createFunction(
	 ...
	 * @param context
	 *         The SKContext for function execution
	 * @return
	 */
	...
	public Mono<SKContext> summarizeConversationAsync(
		@SKFunctionInputAttribute @SKFunctionParameters(description = "A long conversation transcript.", name = "input") String input,
