...
public class TextMemorySkill {
	...
	/// <summary>
	/// Name of the context variable used to specify a unique key associated with
	/// stored
	// information.
	/// </summary>
	...
	/// <summary>
	/// Name of the context variable used to specify the number of memories to
	/// recall
	/// </summary>
	...
	/// {{memory.retrieve }}
	/// </example>
	/// <param name="context">Contains the 'collection' containing the memory to
	/// retrieve and the
	...
	public Mono<String> retrieveAsync(
		SKContext context,
		@SKFunctionParameters(name = collectionParam, description = "Memories collection associated with the memory to retrieve", defaultValue = DefaultCollection) String collection,
		@SKFunctionParameters(name = KeyParam, description = "The key associated with the memory to retrieve") String key) {
		...
		// var key = context.Variables.ContainsKey(KeyParam) ? context[KeyParam] :
		// string.Empty;
		// Verify.NotEmpty(key, "Memory key not defined");

		// context.Log.LogTrace("Recalling memory with key '{0}' from collection '{1}'",
		// key,
		// collection);
	...
	public Mono<SKContext> saveAsync(
		@SKFunctionParameters(name = KeyParam, description = "The information to save", defaultValue = "", type = String.class) String text,
		@SKFunctionParameters(name = collectionParam, description = "Memories collection associated with the information to save", defaultValue = DefaultCollection, type = String.class) String collection,
		@SKFunctionParameters(name = KeyParam, description = "The key associated with the information to save", defaultValue = "", type = String.class) String key,
		SKContext context) {
	 ...
	 * /// <param name="context">Contains the 'collection' to search for the topic
	 * and 'relevance' score</param>
	 ...
	 * [SKFunctionContextParameter(Name = CollectionParam, Description =
	 * "Memories collection to search", DefaultValue = DefaultCollection)]
	 * [SKFunctionContextParameter(Name = RelevanceParam, Description =
	 * "The relevance score, from 0.0 to 1.0, where 1.0 means perfect match",
	 * DefaultValue = DefaultRelevance)]
	 * [SKFunctionContextParameter(Name = LimitParam, Description =
	 * "The maximum number of relevant memories to recall", DefaultValue =
	 * DefaultLimit)]
	 * public string Recall(string text, SKContext context)
	 * {
	 * var collection = context.Variables.ContainsKey(CollectionParam) ?
	 * context[CollectionParam] : DefaultCollection;
	 * Verify.NotEmpty(collection, "Memories collection not defined");
	 * 
	 * var relevance = context.Variables.ContainsKey(RelevanceParam) ?
	 * context[RelevanceParam] : DefaultRelevance;
	 * if (string.IsNullOrWhiteSpace(relevance)) { relevance = DefaultRelevance; }
	 * 
	 * var limit = context.Variables.ContainsKey(LimitParam) ? context[LimitParam] :
	 * DefaultLimit;
	 * if (string.IsNullOrWhiteSpace(limit)) { relevance = DefaultLimit; }
	 * 
	 * context.Log.
	 * LogTrace("Searching memories in collection '{0}', relevance '{1}'",
	 ...
	 * .SearchAsync(collection, text, limitInt, minRelevanceScore:
	 * float.Parse(relevance, CultureInfo.InvariantCulture))
	 * .ToEnumerable();
	 * 
	 * context.Log.LogTrace("Done looking for memories in collection '{0}')",
	 * collection);
	 * 
	 * string resultString;
	 * 
	 ...
	 * else
	 * {
	 * resultString = JsonSerializer.Serialize(memories.Select(x =>
	 * x.Metadata.Text));
	 * }
	 * 
	 ...
	 * /// <param name="context">Contains the 'collection' to save the information
	 * and unique 'key' to associate it with.</param>
	 ...
	 * [SKFunctionContextParameter(Name = KeyParam, Description =
	 ...
	 * [SKFunctionContextParameter(Name = CollectionParam, Description =
	 * "Memories collection associated with the memory to remove",
	 * DefaultValue = DefaultCollection)]
	 * [SKFunctionContextParameter(Name = KeyParam, Description =
	 * "The key associated with the memory to remove")]
