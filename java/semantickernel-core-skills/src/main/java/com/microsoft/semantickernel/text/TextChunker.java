 ...
 * split looking at new
 * lines first, then periods, and so on. For markdown, split looking at
 * punctuation first, and so
 * on.
 */
public class TextChunker {
	...
	private static List<String> internalSplitTextParagraphs(
		...
		// distribute text more evenly in the last paragraphs when the last paragraph is
		// too short.
		if (paragraphs.size() > 1) {
			...
			if (TokenCount(lastParagraph.length()) < maxTokensPerParagraph / 4) {
				List<String> lastParagraphTokens = Arrays.stream(lastParagraph.split(s_spaceChar))
					...
					.collect(Collectors.toList());
				List<String> secondLastParagraphTokens = Arrays.stream(secondLastParagraph.split(s_spaceChar))
					.filter(it -> it.length() != 0)
					.collect(Collectors.toList());
	...
	private static List<String> internalSplitLines(
		...
		SplitString result = split(text, maxTokensPerLine, Collections.singletonList(splitOptions.get(0)), trim);
	...
	private static SplitString split(
		...
		// input.SequenceEqual(inputString.AsSpan()));
	...
	private static int TokenCount(int inputLength) {
		...
		// tokenization
		// strategies
		// depending on the model to be called. For now, we use an extremely rough
		// estimate.
