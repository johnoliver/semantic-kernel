...
public class ActionPlanner {
	...
	public ActionPlanner(Kernel kernel, @Nullable String prompt) {
		...
		this.plannerFunction = SKBuilders.completionFunctions(kernel)
			.createFunction(
				promptTemplate,
				null,
				SkillName,
			...
			.map(
				result -> {
					...
					if (planData.plan.function.contains(".")) {
						...
						CompletionSKFunction skill = context.getSkills()
							.getFunction(
								parts[0],
								parts[1],
	...
	/// <summary>
	/// Native function returning a list of all the functions in the current
	// context,
	/// excluding functions in the planner itself.
	/// </summary>
	/// <param name="goal">Currently unused. Will be used to handle long lists of
	// functions.</param>
	...
	public String listOfFunctions(
		@SKFunctionParameters(name = "goal", description = "The current goal processed by the planner", defaultValue = "") String goal,
		SKContext context) {
