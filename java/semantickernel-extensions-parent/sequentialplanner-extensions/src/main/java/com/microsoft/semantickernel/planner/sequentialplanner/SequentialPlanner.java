...
public class SequentialPlanner {
  ...
  public SequentialPlanner(
      Kernel kernel, @Nullable SequentialPlannerRequestSettings config, @Nullable String prompt) {
        ...
        FunctionBuilders.getCompletionBuilder(kernel)
            .createFunction(
                ...
                new PromptTemplateConfig.CompletionConfig(
                    0.0, 0.0, 0.0, 0.0, this.config.getMaxTokens(), new ArrayList<>()));
        ...
        .flatMap(
            relevantFunctionsManual -> {
              CompletionSKContext updatedContext =
                  context.setVariable("available_functions", relevantFunctionsManual).update(goal);
