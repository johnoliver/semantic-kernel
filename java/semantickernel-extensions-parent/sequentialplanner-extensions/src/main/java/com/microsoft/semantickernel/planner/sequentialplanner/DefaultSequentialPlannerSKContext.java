...
public class DefaultSequentialPlannerSKContext {
	...
	/// <param name="semanticQuery">The semantic query for finding relevant
	/// registered
	// functions</param>
	/// <param name="config">The planner skill config.</param>
	/// <returns>A string containing the manual for all available
	/// functions.</returns>
	...
	/// <summary>
	/// Returns a list of functions that are available to the user based on the
	/// semantic query and
	...
	public Mono<SortedSet<SKFunction<?>>> getAvailableFunctionsAsync(
		...
		List<SKFunction<?>> availableFunctions = functions.stream()
			/*
			 ...
			 */
			.filter(
				s -> !excludedSkills.contains(s.getSkillName())
					&& !excludedFunctions.contains(s.getName()))
			.collect(Collectors.toList());

		Comparator<SKFunction<?>> comparator = Comparator.<SKFunction<?>, String>comparing(SKFunction::getSkillName)
					...
					updatedContext -> {
						...
						return updatedMemory
							.searchAsync(
								...
								false)
							.flatMap(
								memories -> getRelevantFunctionsAsync(
									availableFunctions, memories));
					})
				.map(
					memories -> {
						List<String> added = memories.stream()
							.map(SKFunction::getName)
							.collect(Collectors.toList());

						List<SKFunction<?>> missingFunctions = getMissingFunctions(
	...
	private static List<SKFunction<?>> getMissingFunctions(
		...
		return includedFunctions.stream()
			...
			.flatMap(
				missing -> availableFunctions.stream()
	...
	public Mono<? extends List<? extends SKFunction<?>>> getRelevantFunctionsAsync(
		...
		return Flux.fromIterable(memories)
			.map(
				memoryEntry -> availableFunctions.stream()
					.filter(
						it -> Objects.equals(
		...
		return Flux.fromIterable(availableFunctions)
			.flatMap(
				function -> {
					...
					String description = function.getDescription() == null
