...
public class DefaultSequentialPlannerSKContext {
  ...
  public Mono<String> getFunctionsManualAsync(
    ...
    return functions.map(
        funcs ->
            funcs.stream().map(SKFunction::toManualString).collect(Collectors.joining("\n\n")));
  }
          ...
          .flatMap(
              updatedContext -> {
                ...
                if (updatedMemory == null) {
                  return Mono.just(new ArrayList<SKFunction<Void, CompletionSKContext>>());
                }
                ...
                return updatedMemory
                    .searchAsync(
                        ...
                        false)
                    .flatMap(memories -> getRelevantFunctionsAsync(availableFunctions, memories));
              })
          .map(
              memories -> {
                List<String> added =
                    memories.stream().map(SKFunction::getName).collect(Collectors.toList());

                List<SKFunction<?, ?>> missingFunctions =
                    getMissingFunctions(includedFunctions, availableFunctions, added);
  ...
  private static List<SKFunction<?, ?>> getMissingFunctions(
    ...
    return includedFunctions.stream()
        ...
        .flatMap(missing -> availableFunctions.stream().filter(it -> it.getName().equals(missing)))
        .collect(Collectors.toList());
  }
            ...
            memoryEntry ->
                availableFunctions.stream()
                    .filter(
                        it ->
                            Objects.equals(
                                it.toFullyQualifiedName(), memoryEntry.getMetadata().getId()))
                    .findFirst()
                    .orElse(null))
    ...
    return Flux.fromIterable(availableFunctions)
        .flatMap(
            function -> {
              ...
              String description =
                  function.getDescription() == null || function.getDescription().isEmpty()
                      ? functionName
              ...
              // interface
              return memory
                  .getAsync(PlannerMemoryCollectionName, key, false)
                  .filter(Objects::nonNull)
                  .flatMap(
                      memoryEntry -> {
                        ...
                        return memory.saveInformationAsync(
                            PlannerMemoryCollectionName, textToEmbed, key, description, "");
                      })
                  .flatMap(newKey -> memory.getAsync(PlannerMemoryCollectionName, newKey, true));
            })
